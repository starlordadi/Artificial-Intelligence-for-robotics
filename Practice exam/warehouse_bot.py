...
def plan(warehouse, dropzone, todo):
  move    =      [[-1, 0], # Y,X clockwise starting at UP
                  [-1, 1],
                  [ 0, 1],
                  [ 1, 1],
                  [ 1, 0],
                  ...
                  [ 0,-1],
                  [-1,-1]] 
  cost_move = [1, 1.5, 1, 1.5, 1, 1.5, 1, 1.5]
  cost = 0
  while len(todo) > 0:
    ...
    heuristic = [[99 for row in range(len(warehouse[0]))] for col in range(len(warehouse))]
    ...
    while change:
      ...
      for y in range(len(warehouse)):
        for x in range(len(warehouse[0])):
          ...
          if x == xN and y == yN and heuristic[y][x] == 99:
            heuristic[y][x] = 0
            change = True
          ...
          if warehouse[y][x] == 0 or (x == x0 and y == y0):
            for i in range(len(move)):
              x2 = x + move[i][1]
              y2 = y + move[i][0]
              if x2 >= 0 and x2 < len(warehouse[0]) and y2 >= 0 and y2 < len(warehouse):

                val = heuristic[y2][x2] + cost_move[i]

                if val < heuristic[y][x]:
                  heuristic[y][x] = val
                  change = True

    cost += 2 * heuristic[y0][x0]
    warehouse[yN][xN] = 0
