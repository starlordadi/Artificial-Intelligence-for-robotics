...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        ### ENTER CODE HERE
        dpath = [spath[index + 1][0] - spath[index][0] , spath[index + 1][1] - spath[index][1]]
        Rpath = [estimate[0] - spath[index][0] , estimate[1] - spath[index][1]]
        cte = ((Rpath[1] * dpath[0]) - (Rpath[0] * dpath[1])) / (dpath[1] ** 2 + dpath[0] ** 2)
        U = ((Rpath[0] * dpath[0]) + (Rpath[1] * dpath[1])) / (dpath[0]**2 + dpath[1]**2)
        if U > 1:
            index = index + 1
